...
- **Docker images**: swapped the archived `quay.io/parkervcp/pterodactyl-images` and the ad-hoc `whateverest1/pterodactyl:php7.4` for currently maintained `ghcr.io/pelican-eggs/yolks` / `ghcr.io/pelican-eggs/installers` images (the `parkervcp` org was renamed to `pelican-eggs`; there is no `php` yolk in that project). The XAseco variant runs on the **official** `php:8.2-cli-bookworm` Docker Hub image — an earlier revision used `webdevops/php`, which crashed Wings on start (`failed switching to "root": unable to find user root: no matching entries in passwd file`) because that image intentionally has no `root` entry in `/etc/passwd`. `mysqli` isn't built into the official php image, so the install script compiles it once with `docker-php-ext-install` and copies the resulting `mysqli.so` into the persistent server directory; `start.sh` loads it via `php -d extension=...`.
